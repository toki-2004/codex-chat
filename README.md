...
- **模型列表可实时刷新**：模型下拉框旁的「↻ 刷新」会直接向当前配置档对应供应商的 `/models` 要一份**实时可用模型**（供应商与密钥取自该配置档的 `model_provider` / `[model_providers.<name>]`，密钥只用不落日志、不回传前端）；换配置档时也会自动刷新一次。请求失败（超时 / 401 / 网络不通）不会让下拉框变空——自动回落到本机目录清单，并在下方写明失败原因。
